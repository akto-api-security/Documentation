...
Ensure the instance is accessible from the network where your Apigee API proxy is configured. Note the instance's IP address, as it will be required by the Apigee connector to send agent traffic data to Akto.
...
3. Configure the proxy with the following details:
   ...
   * **Base Path**: Specify the base path for the proxy (e.g., `/api/v1`). This path will be used as the prefix for all requests.
...
* Make a test request through the Apigee proxy.
