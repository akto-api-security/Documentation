...
2.  Add AWS API Gateway connector
    ...
    5\. Find out the \`cloudwatch log group\` for your API gateway for the stage which has the above logs enabled and save it. We'll need it later. 6. Deploy the kubernetes deployment below. 1. For \`LOG\_GROUP\_NAME\` and \`AWS\_REGION\`, use the log group arn we saved earlier and the aws region it is deployed in. 2. For \`AKTO\_KAFKA\_BROKER\_MAL\`, use the value of the \`mini-runtime\` service we deployed in step 1. 3. For \`AWS\_ACCESS\_KEY\_ID\` , \`AWS\_SECRET\_ACCESS\_KEY\` and \`AWS\_SESSION\_TOKEN\`, create AWS CLI keys, which have permissions to read from the cloudwatch log group which we've mentioned above.
