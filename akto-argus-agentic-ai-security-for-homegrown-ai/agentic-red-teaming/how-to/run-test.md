...
{% step %}
(Optional) Edit the **Name** field to identify this scan run. Akto auto-generates a name based on the target and selected tests, but you can rename it or click **Clear selection** to reset the test selection.
{% endstep %}
...
{% step %}
(Optional) Click **Show advance configurations** to add custom headers or conditions to the scan's requests. Choose **Add Header**, enter a key/value pair, and use **Add condition** to chain multiple headers with **AND**/**OR** logic. This is useful for authentication tokens or agent-specific request requirements.
{% endstep %}
